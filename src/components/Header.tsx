        ...
        <div className="flex items-center justify-between h-full">
          ...
          <div className="flex items-center space-x-6 ml-auto">
            <nav className="hidden md:flex items-center space-x-6">
              <div className="relative group">
                ...
                {isServicesOpen && (
                  <div ref={dropdownRef} className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg z-50">
                    <Link to="/services/import-export" onClick={() => { setIsServicesOpen(false); sessionStorage.setItem('fromServices', 'true'); }} className="block px-4 py-3 text-[#002366] hover:bg-[#002366]/10">
                      Import/Export
                    </Link>
                    <Link to="/services/freight-forwarding" onClick={() => { setIsServicesOpen(false); sessionStorage.setItem('fromServices', 'true'); }} className="block px-4 py-3 text-[#002366] hover:bg-[#002366]/10">
                      Freight Forwarding
                    </Link>
                    <Link to="/services/courier" onClick={() => { setIsServicesOpen(false); sessionStorage.setItem('fromServices', 'true'); }} className="block px-4 py-3 text-[#002366] hover:bg-[#002366]/10">
                      Courier Services
                    </Link>
                  </div>
                )}
              ...
              <HashLink smooth to="/#about" className={`font-medium transition-colors duration-300 ${isScrolled ? 'text-[#002366]' : 'text-white'} hover:text-[#FFD700]`}>
                About
              </HashLink>

              <HashLink smooth to="/#contact" className={`font-medium transition-colors duration-300 ${isScrolled ? 'text-[#002366]' : 'text-white'} hover:text-[#FFD700]`}>
                Contact
              </HashLink>
            </nav>
